...
class TicTacToeMod(loader.Module):
    ...
    strings = {
        "name": "TicTacToe",
        "gamestart": (
            "🧠 <b>Game of Tic Tac Toe</b>\n<i>Waiting for second"
            " player</i>"
        ),
        "gamestart_ai": " <b>Bot is ready to compete</b>",
    ...
    strings_ru = {
        "gamestart": (
            "🧠 <b>Игра в крестики-нолики</b>\n<i>Ожидание второго"
            " игрока</i>"
        ),
        "gamestart_ai": "<b>Бот готов сражаться</b>",
        ...
        "ai_game": (
            "🧠 <b>{}</b>\n<i><b>{}</b> играет с <b>ботом</b></i>\n\n<i>Ты {}</i>"
        ),
        "not_with_yourself": "Ты не можешь играть сам с собой!",
        "_cmd_doc_tictactoe": "- начать новую игру",
        "_cmd_doc_tictacai": "- сыграть против бота",
        "_cls_doc": "Игра в крестики-нолики в Телеграм",
    }
...
{board[2][0]} | {board[2][1]} | {board[2][2]}"""
    ...
    async def inline__start_game_ai(self, call: InlineCall):
        ...
        first = choice(["bot", user.id])
        self._games[uid] = {
            "2_player": "bot",
            "turn": user.id,
            "mapping": {first: "x", "bot" if first != "bot" else user.id: "o"},
            "amifirst": first == user.id,
            "user": user,
            "ai_player": Player.x if first == "bot" else Player.o,
            "human_player": Player.o if first == "bot" else Player.x,
            "bot": AbBot(Player.x if first == "bot" else Player.o),
            "board": Board(),
        }
