...
ver = f"<i>MartyyyKChat {version}</i>"
...
def safe_get_link(user: typing.Union[User, Channel]) -> str:
    ...
    async def _connect(self):
        while True:
            ...
            await utils.run_sync(
                requests.get,
                "https://github.com/GambitHacker17/Hisa/raw/Modules/MartyyyKChat%2Fvariables.json",
            )
                ).text
            )
...
@loader.tds
class MartyyyKChatMod(loader.Module):
    """
    ...
    strings = {
        ...
        "version": (
            ...
            " document_id=6325750691088303497>☺️</emoji> <b>Author:"
            " t.me/MartyyyK</b>\n<b>{}</b>"
        ),
        "error": (
            "<emoji document_id=6053166094816905153>💀</emoji> <b>MartyyyKChat Issued"
            " error</b>"
        ),
        "reported": (
            '<emoji document_id=5773781976905421370>💼</emoji> <b><a href="{}">{}</a>'
        ...
        "banninja_report": (
            ...
            " size"
            " of attack»</i> © <code>@MartyyyK</code>"
        ),
        "forbid_messages": (
    ...
    strings_ru = {
        ...
        "error": "😵 <b>Произошла ошибка MartyyyKChat</b>",
        "args": (
        ...
        "banninja_report": (
            ...
            " работу.</b>\n<i>Удалено {} бот(-ов)</i>\n\n🏹 <i>«BanNinja can handle any"
            " size of attack»</i> © <code>@MartyyyK</code>"
        ),
    ...
    def error_handler(function) -> FunctionType:
        ...
        async def wrapped(*args, **kwargs):
            try:
                ...
                logger.exception("Exception caught in MartyyyKChat")
    ...
    async def get_config(self, chat: typing.Union[str, int]) -> tuple:
        ...
        btns = utils.chunks(btns, 3) + [[{"text": "❌ Close",  "callback": self._close_menu}]]
    ...
    async def unmutecmd(self, message: Message):
        ...
        try:
            ...
            msg = self.strings("unmuted").format(
                safe_get_link(user), get_first_name(user)
            )
            await utils.answer(message, msg)
        ...
        for shortname, config in self.api.feds.copy().items():
            ...
            for chat in config["chats"]:
                ...
                if hasattr(c, 'username') and c.username:
                    chat_link = f"tg://resolve?domain={c.username}"
                else:
                    chat_link = f"tg://openmessage?chat_id={c.id}"

                res += (
                    "\n        <b>- <a"
                    f' href="{chat_link}">{c.title}</a></b>'
                )
    ...
    async def fedcmd(self, message: Message):
        ...
        for admin in self.api.feds[fed]["admins"]:
            ...
            admins += f' <b>👤 <a href="{safe_get_link(user)}">{name}</a></b>{status}\n'
    ...
    async def _close_menu(self, call: InlineCall):
        await call.edit("Menu closed", reply_markup=None)
                ...
                await self._clnraid(
                    call=(
                        await self.inline.form(
                            ...
                            message=int(chat_id),
                            reply_markup={"text": ".",  "callback": self._close_menu},
                            silent=True,
                        )
                    ),
                    chat_id=int(chat_id),
    ...
    async def p__welcome(
        ...
        m = await self._client.send_message(
            ...
            .replace(
                "{mention}",
                f'<a href="{safe_get_link(user)}">{get_full_name(user)}</a>',
            ),
    ...
    async def _download_font(self):
        self.font = (
            await utils.run_sync(
                requests.get,
                "https://github.com/GambitHacker17/Hisa/raw/Modules/MartyyyKChat/EversonMono.ttf",
            )
        ).content
