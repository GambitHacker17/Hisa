...
from hisatl.tl.types import PeerUser
...
class Chess(loader.Module):
    ...
    async def backToInvite(self,call,nT):
        if call.from_user.id not in self.you_n_me:
            ...
            return
        await call.edit(text = f"<a href='tg://user?id={self.opp_id}'>{self.opp_name}</a> приглашен сыграть в шахматы\n-- --\n[⚙️] Текущие настройки:\n| - > • Хост играет за {self.colorName} цвет\n| - > • Время: {self.timeName}", 
                               reply_markup = [
                                   [
    ...
    async def time(self, call, nT):
        ...
        await call.edit(
            ...
            reply_markup=[
                [
                    {"text":"⚡ Блиц","action":"answer","message":"Блиц - скорость без границ"}
                ],
                [
    ...
    async def chess(self, message):
        ...
        if isinstance(message.peer_id, PeerUser):
            ...
            if message.is_reply:
                ...
                if len(args)==0:
                    await message.edit("<emoji document_id=5370724846936267183>🤔</emoji> В группе нужно указать с кем играть (reply/username/id)")
                    return
                opponent = args[0]
                try:
                    if opponent.isdigit():
    ...
    async def ans(self, call, data):
        if call.from_user.id == self.message.sender_id:
            ...
            return
        if call.from_user.id not in self.you_n_me:
            await call.answer("Не тебе предлагают")
            return
        if data == 'y':
            self.Board = chess.Board()
            if not self.you_play:
                await call.edit(text="Сознание сторон")
            ...
            await asyncio.sleep(0.5)
            if self.pTime:
                await call.edit(text="Установка таймеров")
