...
from hisatl.errors import FloodWaitError, RPCError
...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.hisa_me.id
        self._cached_usernames = [
            (
                self._client.hisa_me.username.lower()
                if self._client.hisa_me.username
                else str(self._client.hisa_me.id)
            )
        ]

        self._cached_usernames.extend(
            getattr(self._client.hisa_me, "usernames", None) or []
        )
    ...
    async def _handle_command(
        ...
        if (
            ...
            and message.chat.title.startswith("hisa-")
            and message.chat.title != "hisa-logs"
        ):
            if not watcher:
