...
from hisatl.errors.rpcerrorlist import YouBlockedUserError
...
class TokenObtainment(InlineUnit):
    async def _create_bot(self):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("hisa.inline", "custom_bot", False):
                username = self._db.get("hisa.inline", "custom_bot").strip("@")
                username = f"@{username}"
                ...
                try:
                    ...
                    username = f"@hisa{user_id}_bot"
            else:
                username = f"@hisa{user_id}_bot"

            for msg in [
                f"🌘 Hisa Userbot of {self._name}"[:64],
                username,
                "/setuserpic",
                username,
            ]:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    if self._db.get("hisa.inline", "custom_bot", False) and self._db.get("hisa.inline", "custom_bot", False) != button.text.strip("@"):
                        continue

                    if not self._db.get("hisa.inline", "custom_bot", False) and not re.search(rf"@hisa{user_id}_bot", button.text):
                        continue
                    ...
                    self._db.set("hisa.inline", "bot_token", token)
                    self._token = token
    ...
    async def _dp_revoke_token(self, already_initialised: bool = True):
        ...
        self._db.set("hisa.inline", "bot_token", None)
        self._token = None
