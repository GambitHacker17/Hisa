...
from hisatl.network.connection import (
    ConnectionTcpFull,
    ConnectionTcpMTProxyRandomizedIntermediate,
)
from hisatl.password import compute_check
from hisatl.sessions import MemorySession, SQLiteSession
...
class Hisa:
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: f.startswith("hisa-") and f.endswith(".session"),
                os.listdir(BASE_DIR),
            )
        ]
    ...
    async def save_client_session(self, client: CustomTelegramClient):
        ...
        client.hisa_db = database.Database(client)
        await client.hisa_db.init()
    ...
    async def _badge(self, client: CustomTelegramClient):
        try:
            ...
            await client.hisa_inline.bot.send_photo(
                logging.getLogger().handlers[0].get_logid_by_client(client.tg_id),
                "https://raw.githubusercontent.com/GambitHacker17/Hisa/Master/Hisa_started.png",
                caption=(
                    f"🌘 <b>Hisa Userbot started!</b>\n\n🌳 <b>Creator: @MartyyyK</b>\n\n"
                    f"• Build: {BUILD}\n"
                    f"• Version: {VERSION}\n"
                    f"• {UPD}\n"
                ),
            ...
            logging.debug(
                ...
                client.tg_id,
                client.hisa_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
...
hisatl.extensions.html.CUSTOM_EMOJIS = not get_config_key("disable_custom_emojis")
