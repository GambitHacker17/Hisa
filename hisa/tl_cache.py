...
from hisatl import TelegramClient
from hisatl.errors.rpcerrorlist import TopicDeletedError
from hisatl.hints import EntityLike
from hisatl.network import MTProtoSender
from hisatl.tl.functions.channels import GetFullChannelRequest
from hisatl.tl.functions.users import GetFullUserRequest
from hisatl.tl.tlobject import TLRequest
from hisatl.tl.types import (
    ChannelFull,
    Message,
    Updates,
    UpdatesCombined,
...
from hisatl.utils import is_list_like
...
class CustomTelegramClient(TelegramClient):
    def __init__(self, *args, **kwargs):
        ...
        self._hisa_entity_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordEntity,
        ] = {}

        self._hisa_perms_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordPerms,
        ] = {}

        self._hisa_fullchannel_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordFullChannel,
        ] = {}

        self._hisa_fulluser_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordFullUser,
        ] = {}
    ...
    @property
    def hisa_entity_cache(self) -> typing.Dict[int, CacheRecordEntity]:
        return self._hisa_entity_cache

    @property
    def hisa_perms_cache(self) -> typing.Dict[int, CacheRecordPerms]:
        return self._hisa_perms_cache

    @property
    def hisa_fullchannel_cache(self) -> typing.Dict[int, CacheRecordFullChannel]:
        return self._hisa_fullchannel_cache

    @property
    def hisa_fulluser_cache(self) -> typing.Dict[int, CacheRecordFullUser]:
        return self._hisa_fulluser_cache
    ...
    async def get_entity(
        ...
        :return: :obj:`Entity`
        """
        _hisa_client_id_logging_tag = copy.copy(self.tg_id)
        ...
        if (
            not force
            and hashable_entity
            and hashable_entity in self._hisa_entity_cache
            and (
                not exp
                or self._hisa_entity_cache[hashable_entity].ts + exp > time.time()
            )
        ):
            logger.debug(
                "Using cached entity %s (%s)",
                entity,
                type(self._hisa_entity_cache[hashable_entity].entity).__name__,
            )
            return copy.deepcopy(self._hisa_entity_cache[hashable_entity].entity)
        ...
        if resolved_entity:
            ...
            self._hisa_entity_cache[hashable_entity] = cache_record
            ...
            if getattr(resolved_entity, "id", None):
                ...
                self._hisa_entity_cache[resolved_entity.id] = cache_record
            ...
            if getattr(resolved_entity, "username", None):
                ...
                self._hisa_entity_cache[f"@{resolved_entity.username}"] = cache_record
                self._hisa_entity_cache[resolved_entity.username] = cache_record
    ...
    async def get_perms_cached(
        ...
        if (
            ...
            and hashable_entity
            and hashable_user
            and hashable_user in self._hisa_perms_cache.get(hashable_entity, {})
            and (
                not exp
                or self._hisa_perms_cache[hashable_entity][hashable_user].ts + exp
                > time.time()
            )
        ):
            logger.debug("Using cached perms %s (%s)", hashable_entity, hashable_user)
            return copy.deepcopy(
                self._hisa_perms_cache[hashable_entity][hashable_user].perms
            )
        ...
        if resolved_perms:
            ...
            self._hisa_perms_cache.setdefault(hashable_entity, {})[
                hashable_user
            ] = cache_record
            ...
            def save_user(key: typing.Union[str, int]):
                ...
                if getattr(user, "id", None):
                    self._hisa_perms_cache.setdefault(key, {})[user.id] = cache_record

                if getattr(user, "username", None):
                    self._hisa_perms_cache.setdefault(key, {})[
                        f"@{user.username}"
                    ] = cache_record
                    self._hisa_perms_cache.setdefault(key, {})[
                        user.username
    ...
    async def get_fullchannel(
        ...
        if (
            not force
            and self._hisa_fullchannel_cache.get(hashable_entity)
            and not self._hisa_fullchannel_cache[hashable_entity].expired
            and self._hisa_fullchannel_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._hisa_fullchannel_cache[hashable_entity].full_channel

        result = await self(GetFullChannelRequest(channel=entity))
        self._hisa_fullchannel_cache[hashable_entity] = CacheRecordFullChannel(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def get_fulluser(
        ...
        if (
            not force
            and self._hisa_fulluser_cache.get(hashable_entity)
            and not self._hisa_fulluser_cache[hashable_entity].expired
            and self._hisa_fulluser_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._hisa_fulluser_cache[hashable_entity].full_user

        result = await self(GetFullUserRequest(entity))
        self._hisa_fulluser_cache[hashable_entity] = CacheRecordFullUser(
            hashable_entity,
            result,
            exp,
        )
