...
class TestMod(loader.Module):
        ...
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                ...
                (
                    ...
                    " Hisa will try to determine, which client caused logs. E.g. there"
                    ...
                    " Hisa will send all logs to Client1 and Client2, even if it is"
    ...
    async def logs(
        ...
        if not isinstance(lvl, int):
            try:
                if not self.inline.init_complete or not await self.inline.form(
                    ...
                    + [[{"text": self.strings("cancel"), "callback": self._canceled}]],
                    message=message,
                ):
                    raise
            except Exception:
        ...
        other = (
            *main.__version__,
            (
                " <a"
                f' href="https://raw.githubusercontent.com/GambitHacker17/Hisa/Master/{ghash}">@{ghash[:8]}</a>'
                if ghash
                else ""
            ),
        )
    ...
    async def _canceled(self, call: InlineCall):
    ...
    async def client_ready(self):
        chat, _ = await utils.asset_channel(
            self._client,
            "hisa-logs",
            "🌘 Your Hisa logs will appear in this chat",
            silent=True,
            invite_bot=True,
            avatar="https://raw.githubusercontent.com/GambitHacker17/Hisa/Master/hisa-logs.png",
        )
