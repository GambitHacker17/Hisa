...
from hisatl.tl.types import Message, PeerUser, User
...
@loader.tds
class HisaSecurityMod(loader.Module):
    """Control security settings"""

    strings = {"name": "HisaSecurity"}
    ...
    def _build_markup(
        ...
        return (
            ...
            + [[{"text": self.strings("close_menu"), "callback": self._sec_menu_close}]]
            if is_inline
            else utils.chunks(
                [
                    {
    ...
    async def _sec_menu_close(self, call: InlineCall):
        await call.edit(self.strings("sec_menu_close"), reply_markup=None)
    ...
    async def _confirm(
        ...
        await utils.answer(
            ...
            reply_markup=[
                ...
                {"text": self.strings("cancel_btn"), "callback": self._close_menu},
            ],
        )
