...
import hisatl
from hisatl.tl.functions.channels import JoinChannelRequest
from hisatl.tl.functions.messages import (
    GetDialogFiltersRequest,
    UpdateDialogFilterRequest,
)
from hisatl.tl.types import Message
from hisatl.utils import get_display_name
...
@loader.tds
class HisaSettingsMod(loader.Module):
    """Advanced settings for Hisa Userbot"""

    strings = {"name": "HisaSettings"}
    ...
    async def _uninstall(self, call: InlineCall):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"hisa-logs", "silent-tags"}
                )
                or (
        ...
        if any(folder.title == "hisa" for folder in folders):
            folder_id = max(
                folders,
                key=lambda x: x.id,
            ).id
    ...
    async def _btn_no(self, call: InlineCall):
        await call.edit(self.strings("op_canceled"), reply_markup=None)
        
    async def _close_menu(self, call: InlineCall):
        await call.edit(self.strings("menu_closed"), reply_markup=None)

    async def cmd_settings(self, call: InlineCall):
        await call.edit(
    ...
    @loader.command()
    async def uninstall_hisa(self, message: Message):
        await self.inline.form(
            ...
            [
                ...
                {"text": self.strings("deauth_cancel"), "callback": self._btn_no},
            ],
        )
    ...
    async def inline__setting(self, call: InlineCall, key: str, state: bool = False):
        if callable(key):
            key()
            hisatl.extensions.html.CUSTOM_EMOJIS = not main.get_config_key(
                "disable_custom_emojis"
            )
        else:
            if key == "debugger":
                self._db.set(log.__name__, key, state)
            else:
    ...
    async def remove_core_protection(self, message: Message):
        ru_doc = "⚠️ Отключить защиту ядра (необратимо). Позволяет изменять системные модули"
        en_doc = "⚠️ Disable core protection (irreversible). Allows modifying core modules"
    ...
    remove_core_protection.ru_doc = "Отключить защиту ядра (необратимо). Позволяет изменять системные модули"
    remove_core_protection.en_doc = "Disable core protection (irreversible). Allows modifying core modules"
        ...
        if confirm_required:
            await call.edit(
                ...
                reply_markup=[
                    ...
                    {"text": "🚫 Cancel", "callback": self.cmd_settings,},
                ],
            )
            return
    ...
    def _get_settings_markup(self) -> list:
        return [
            ...
            [{"text": self.strings("close_menu"), "callback": self._close_menu}]
        ]
    ...
    async def weburl(self, message: Message, force: bool = False):
        if "LAVHOST" in os.environ:
            form = await self.inline.form(
                ...
                reply_markup={
                    ...
                    "url": await main.hisa.web.get_url(proxy_pass=False),
                },
                gif="",
            )
            return
            ...
            try:
                if not await self.inline.form(
                    ...
                    reply_markup=[
                        ...
                        {"text": self.strings("btn_no"), "callback": self._btn_no},
                    ],
        ...
        if not main.hisa.web:
            main.hisa.web = core.Web(
                data_root=main.BASE_DIR,
                api_token=main.hisa.api_token,
                proxy=main.hisa.proxy,
                connection=main.hisa.conn,
            )
            await main.hisa.web.add_loader(self._client, self.allmodules, self._db)
            await main.hisa.web.start_if_ready(
                len(self.allclients),
                main.hisa.arguments.port,
                proxy_pass=main.hisa.arguments.proxy_pass,
            )
        ...
        url = await main.hisa.web.get_url(proxy_pass=True)
    ...
    async def invokecmd(self, message: Message):
        ...
        if module == "core":
            if method == "flush_entity_cache":
                result = (
                    f"Dropped {len(self._client._hisa_entity_cache)} cache records"
                )
                self._client._hisa_entity_cache = {}
            elif method == "flush_fulluser_cache":
                result = (
                    f"Dropped {len(self._client._hisa_fulluser_cache)} cache records"
                )
                self._client._hisa_fulluser_cache = {}
            elif method == "flush_fullchannel_cache":
                result = (
                    f"Dropped {len(self._client._hisa_fullchannel_cache)} cache"
                    " records"
                )
                self._client._hisa_fullchannel_cache = {}
            elif method == "flush_perms_cache":
                result = f"Dropped {len(self._client._hisa_perms_cache)} cache records"
                self._client._hisa_perms_cache = {}
                ...
                result = (
                    f"Dropped {len(self._client._hisa_entity_cache)} entity cache"
                    " records\nDropped"
                    f" {len(self._client._hisa_fulluser_cache)} fulluser cache"
                    " records\nDropped"
                    f" {len(self._client._hisa_fullchannel_cache)} fullchannel cache"
                    " records\nDropped"
                ...
                self._client._hisa_fulluser_cache = {}
                self._client._hisa_fullchannel_cache = {}
                self._client.hisa_me = await self._client.get_me()
                ...
                result = (
                    "Entity cache:"
                    f" {len(self._client._hisa_entity_cache)} records\nFulluser cache:"
                    f" {len(self._client._hisa_fulluser_cache)} records\nFullchannel"
                    " cache:"
                    f" {len(self._client._hisa_fullchannel_cache)} records\nLoader"
