...
@loader.tds
class HisaBackupMod(loader.Module):
    """Handles database and modules backups"""

    strings = {"name": "HisaBackup"}

    async def client_ready(self):
        if not self.get("period"):
            await self.inline.bot.send_photo(
                self.tg_id,
                photo="https://raw.githubusercontent.com/GambitHacker17/Hisa/Master/Unit_alpha.png",
        ...
        self._backup_channel, _ = await utils.asset_channel(
            ...
            silent=True,
            archive=True,
            avatar="https://raw.githubusercontent.com/GambitHacker17/Hisa/Master/Hisa-backups.png",
            _folder="hisa",
        )
    ...
    async def handler(self):
        try:
            ...
            backup.name = (
                f"hisa-db-backup-{datetime.datetime.now():%d-%m-%Y-%H-%M}.json"
            )
            ...
            logger.exception("HisaBackup failed")
    ...
    async def restoredb(self, message: Message):
        ...
        with contextlib.suppress(KeyError):
            decoded_text["hisa.inline"].pop("bot_token")
