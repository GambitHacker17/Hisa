...
import requests
from hisatl.errors.rpcerrorlist import MediaCaptionTooLongError
from hisatl.tl.functions.channels import JoinChannelRequest
from hisatl.tl.types import Channel, Message
...
class LoaderMod(loader.Module):
    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                "MODULES_REPO",
                "https://github.com/GambitHacker17/Hisa/raw/Modules",
    ...
    async def _async_init(self):
        modules = list(
            filter(
                lambda x: not x.startswith("https://github.com/GambitHacker17/Hisa/raw/Modules"),
                utils.array_sum(
                    map(
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?hisa_min", doc):
            ver = re.search(r"# ?scope: ?hisa_min ((?:\d+\.){2}\d+)", doc).group(1)
        ...
        if is_dragon:
            ...
            module_name = f"hisa.modules.{uid}"
            doc = geek.compat(doc)
        ...
        async with (dragon.import_lock if is_dragon else lambda _: FakeLock())(
                ...
                try:
                    try:
                        ...
                        async def inner_proxy():
                            ...
                            while True:
                                if hasattr(instance, "hisa_wait_channel_approve"):
                                    if message:
                                        (
                                            module,
                                            channel,
                                            reason,
                                        ) = instance.hisa_wait_channel_approve
                                        message = await utils.answer(
                                            message,
            ...
            try:
                developer_entity = await (
                    ...
                    if (
                        developer in self._client.hisa_entity_cache
                        and getattr(
