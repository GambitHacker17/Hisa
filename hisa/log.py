...
def getlines(filename: str, module_globals=None) -> str:
    ...
    source code of Hisa and Dragon modules properly. This is needed for
    ...
    try:
        if filename.startswith("<") and filename.endswith(">"):
            ...
            if (
                module.startswith("hisa.modules")
                ...
                if isinstance(value, dict):
                    ...
                    try:
                        if (
                            ...
                            value,
                            (hisatl.TelegramClient, CustomTelegramClient),
                        ):
...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.hisa_caller or client_id == record.hisa_caller)
        ]
    ...
    async def _show_full_trace(
        ...
        chunks = list(utils.smart_split(*hisatl.extensions.html.parse(chunks), 4096))
    ...
    def _gen_web_debug_button(self, item: HisaException) -> list:
        if not item.sysinfo:
            return []
        ...
        async with self._send_lock:
            ...
            self._exc_queue = {
                client_id: [
                    ...
                    for item in self.tg_buff
                    if isinstance(item[0], HisaException)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_hisa_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_hisa_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        record.hisa_caller = caller

        if record.levelno >= self.tg_level:
            if record.exc_info:
                exc = HisaException.from_exc_info(
                    *record.exc_info,
