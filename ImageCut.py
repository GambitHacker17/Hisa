...
class AmeImageCutMod(loader.Module):
    ...
    async def cut(self, message):
        """<кол-во по вертикали> <кол-во по горизонтали>"""
        imagesinzip = int() 
        args = utils.get_args(message)
        await message.edit("<emoji document_id=5220046725493828505>✍️</emoji> <b>Ожидайте...</b>") 
        if len(args) != 2:
            await message.edit("<emoji document_id=5220197908342648622>❗</emoji> <b>Укажите количество частей по вертикали и горизонтали")
            return
        ...
        await self.client.send_file(
            message.chat_id,
            file=archive,
            caption=f"<emoji document_id=5222148368955877900>🔥</emoji> <b>Количество фотографий: {imagesinzip}.</b>",
            force_document=True,
            reply_to=reply,
